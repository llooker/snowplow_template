  ...
  derived_table:
    ...
    sql: |
      select distinct domain_userid || domain_sessionidx as session_pkey
        ...
        , first_value((CASE WHEN mkt_content = '' OR refr_medium = 'internal' THEN NULL ELSE mkt_content END) ignore nulls) over (partition by domain_userid, domain_sessionidx order by dvce_tstamp rows between unbounded preceding and unbounded following) as mkt_content
